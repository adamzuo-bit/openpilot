# 文档索引

## OP Agent 产品

| 文档 | 说明 |
|------|------|
| [PRODUCT.md](PRODUCT.md) | 产品定位与北极星 |
| [QUICKSTART.md](QUICKSTART.md) | 5 分钟上手 |
| [CLI.md](CLI.md) | `op` 命令行 |
| [TERMINAL.md](TERMINAL.md) | Web 终端 + Hermes 风格 op |
| [CAPABILITIES.md](CAPABILITIES.md) | 能做什么 |
| [FAQ.md](FAQ.md) | 常见问题 |
| [TUNING_GUIDE.md](TUNING_GUIDE.md) | 调参流程 |
| [TROUBLESHOOTING.md](TROUBLESHOOTING.md) | 排障 |
| [WIKI.md](WIKI.md) | GitHub Wiki 同步说明 |
| [wiki/](wiki/) | **Wiki 源稿**（发布到 github.com/mouxangithub/ai/wiki） |

## 用户与部署

| 文档 | 说明 |
|------|------|
| [INSTALL.md](INSTALL.md) | 安装、集成、更新、卸载 |
| [OVERVIEW.md](OVERVIEW.md) | Web 功能、API 端点 |
| [COMMA_DEVICES.md](COMMA_DEVICES.md) | C2/C3/C3X/C4 |
| [PANDA_FLASH.md](PANDA_FLASH.md) | Panda 刷机 |
| [GITHUB_RUNNER.md](GITHUB_RUNNER.md) | C3 GitHub Runner |

## 反馈与发布

| 文档 | 说明 |
|------|------|
| [ISSUES.md](ISSUES.md) | Issue 提交 |
| [GIT_PR.md](GIT_PR.md) | 发 PR |
| [PUBLISH.md](PUBLISH.md) | 多平台发布 |
| [PR_AUTOMATION.md](PR_AUTOMATION.md) | CI 自动审阅 |

## 架构与开发

| 文档 | 说明 |
|------|------|
| [ARCHITECTURE.md](ARCHITECTURE.md) | **分层架构**（server / core / services / tools/domains） |
| [ARCHITECTURE_TARGETS.md](ARCHITECTURE_TARGETS.md) | 迁移状态、依赖规则、脚本清单 |
| [TSK_AND_AID.md](TSK_AND_AID.md) | aid 进程与 SecOC |
| [VEHICLE_ADAPTATION_GUIDE.md](VEHICLE_ADAPTATION_GUIDE.md) | 新车适配 |
| [HERMES_EVOLUTION.md](HERMES_EVOLUTION.md) | 技能进化闭环 |
| [SKILL_AUTHORING.md](SKILL_AUTHORING.md) | 技能编写 |
| [PLUGIN_DEV.md](PLUGIN_DEV.md) | 插件 |
| [FORK_AND_COMMUNITY.md](FORK_AND_COMMUNITY.md) | Fork 与 Wiki RAG |
| [SESSION_SYNC.md](SESSION_SYNC.md) | 会话同步（`core/sync/hub.py`） |
| [UPSTREAM_SYNC.md](UPSTREAM_SYNC.md) | 上游同步 |
| [../dev/README.md](../dev/README.md) | PC 预览与测试 |

## GitHub

- 仓库：https://github.com/mouxangithub/ai  
- Wiki：https://github.com/mouxangithub/ai/wiki  
- Issue 模板：`.github/ISSUE_TEMPLATE/`  
- PR 模板：`.github/pull_request_template.md`  
- 贡献：[CONTRIBUTING.md](../.github/CONTRIBUTING.md)
