#!/usr/bin/env python3
"""DEPRECATED: use opencode-pr.yml (review) + ai_pr_safe_merge.py (merge). Kept for reference."""
...
# Same presets as ai/client.py — endpoint resolved from GitHub Actions vars/secrets.
DEFAULT_ENDPOINTS = {
...
def _github_ai_config() -> dict[str, str]:
  """Read LLM config from GitHub Actions secrets/variables (not device config.json).

  Configure in repo Settings → Secrets and variables → Actions, or in Environment
  `ai-pr-review` when the workflow uses `environment: ai-pr-review`.

  | Kind | Name | Maps to op助手 |
  |------|------|----------------|
  | Secret | AI_API_KEY | ai_api_key |
  | Variable | AI_PROVIDER | ai_provider (default opencode-zen) |
  | Variable | AI_MODEL | ai_model (default deepseek-v4-flash) |
  | Variable | AI_BASE_URL | ai_base_url (custom / optional override) |
  | Variable | AI_TEMPERATURE | ai_temperature (default 0.3 for review) |
  """
...
def _llm_review_body(pull: dict, files: list[dict], analysis: dict, *, kind: str) -> tuple[str, str]:
  """Returns (body, mode) where mode is 'llm' or 'rules'."""
  ...
  system = (
    ...
    "openpilot 仓禁止改动 selfdrive/panda/opendbc safety 等；assistant 仓可更宽松。"
  ...
  user = "\n".join([
    ...
    f"blocked_paths: {analysis.get('blocked_paths', [])}",
    "",
    "PR 描述:",
  ...
  if choices:
    ...
    if content.strip():
      ...
      footer = [
        "",
        "---",
        f"规则门控：自动合并资格 **{'是' if analysis.get('auto_merge_eligible') else '否'}**（需 `{LABEL_SAFE_MERGE}` + CI 绿）",
      ]
...
def main() -> int:
  ...
  print(f"review mode: {review_mode} (provider={_github_ai_config().get('provider')}, model={_github_ai_config().get('model')})")
  review = _api(
    "POST",
  ...
  auto_merge = _env("AI_AUTO_MERGE", "1") == "1"
  if not auto_merge:
